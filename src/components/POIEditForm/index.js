...
class POIEditForm extends Component {
    ...
    onSubmit = event => {
        const { name, longitude, latitude, description, fileupload, imageList, audioList, imageArray } = this.state;
      ...
      if (this.state.isModalOpen) {
        ...
        return this.state.imageArray.map(image =>
          ...
          <Col md="6" className="img-col" key={image.url.split("token=")[1]}>
            <Card>
              <CardImg className="card-img-top" src={image.url} alt="Image name here" 
                onClick={() => this.toggleFile(image.metadata)}/>
    ...
    render() {
        ...
        return (
            <Fragment>
                ...
                {/* File edit screen, includes name, description, delete button */}
